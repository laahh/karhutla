window.KarhutlaDopStore = (function () {
  const API = "/api/dop";
  const PHOTO_API = "/api/laporan-photo";
  const DB_NAME = "karhutla-dop-v1";
  const DB_VER = 1;
  const displayCache = {};
  ...
  async function hydrate() {
    ...
    await cacheIdbPhotos(window.KARHUTLA_DOP_DATA);
    return window.KARHUTLA_DOP_DATA;
  }
  ...
  async function save(data) {
    applyData(data);
    await idbSetData(window.KARHUTLA_DOP_DATA);
    try {
      ...
      if (got.json && got.json.ok && got.json.data) {
        ...
        return { ok: true, persistent: true, data: window.KARHUTLA_DOP_DATA };
      }
    } catch (err) {
      /* keep local copy */
    }
    return { ok: true, persistent: false, data: window.KARHUTLA_DOP_DATA };
  }
  ...
  async function uploadPhoto(file, name) {
    ...
    try {
      ...
      const got = await fetchApi(PHOTO_API, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: name || (file && file.name) || "dop.jpg",
