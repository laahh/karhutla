const BLOB_KEY = "karhutla/kompetensi-data.json";
...
function normalize(data) {
  ...
  data.source_file = data.source_file || "Form input kompetensi personil KARHUTLA";
...
async function readStore() {
  ...
  try {
    ...
    const listed = await list({ prefix: "karhutla/kompetensi-data", limit: 20 });
    const hit = (listed.blobs || []).find(function (blob) {
      return blob.pathname === BLOB_KEY || blob.pathname.indexOf("karhutla/kompetensi-data") === 0;
    });
    if (hit && hit.url) {
      const res = await fetch(hit.url, { cache: "no-store" });
      if (res.ok) {
        return { persistent: true, data: normalize(await res.json()) };
      }
    }
...
function sortRecords(records) {
  return records.slice().sort(function (a, b) {
    const ka = String((a && a.tanggal_verifikasi) || "") + " " + String((a && a.personil) || "");
    const kb = String((b && b.tanggal_verifikasi) || "") + " " + String((b && b.personil) || "");
  ...
  try {
    ...
    if (action === "replace") {
      ...
      if (!incoming || !Array.isArray(incoming.records)) {
        send(res, 400, { ok: false, error: "Data kompetensi tidak valid." });
        return;
      }
    ...
    send(res, 503, {
      ok: false,
      persistent: false,
      error: err && err.message ? err.message : "Gagal memproses kompetensi."
    });
  }
};
