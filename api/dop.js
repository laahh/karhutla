const BLOB_KEY = "karhutla/dop-data.json";
...
function normalize(data) {
  ...
  data.source_file = data.source_file || "Form input DOP Karhutla";
...
async function readStore() {
  ...
  try {
    ...
    const listed = await list({ prefix: "karhutla/dop-data", limit: 20 });
    const hit = (listed.blobs || []).find(function (blob) {
      return blob.pathname === BLOB_KEY || blob.pathname.indexOf("karhutla/dop-data") === 0;
    });
    if (hit && hit.url) {
  ...
  try {
    ...
    if (action === "replace") {
      ...
      if (!incoming || !Array.isArray(incoming.records)) {
        send(res, 400, { ok: false, error: "Data DOP tidak valid." });
        return;
      }
    ...
    send(res, 503, {
      ok: false,
      persistent: false,
      error: err && err.message ? err.message : "Gagal memproses DOP."
    });
  }
};
