(function () {
  ...
  function dataStore() {
    if (!window.KARHUTLA_DOP_DATA || !Array.isArray(window.KARHUTLA_DOP_DATA.records)) {
      window.KARHUTLA_DOP_DATA = {
        source_file: "Form input DOP Karhutla",
        total_records: 0,
        records: []
      };
    }
    return window.KARHUTLA_DOP_DATA;
  }
  ...
  function addPhotoFiles(fileList) {
    ...
    incoming.forEach(function (file) {
      ...
      if (existingPhotos.length + pendingPhotos.length >= PHOTO_MAX) {
        setStatus("Maksimal " + PHOTO_MAX + " foto per DOP.", "is-err");
        return;
      }
      pendingPhotos.push({
        file: file,
  ...
  function fillForm(row, index) {
    ...
    existingPhotos = files.filter(function (src) {
      ...
      const shown = window.KarhutlaDopStore ? (KarhutlaDopStore.displayUrl(src) || src) : src;
    ...
    if (index >= 0) {
      titleEl.textContent = "Ubah DOP";
      hintEl.textContent = "Mengubah data yang sudah tersimpan. Klik Simpan.";
    } else {
      titleEl.textContent = "DOP baru";
      hintEl.textContent = "Lengkapi data DOP. Kolom bertanda * wajib diisi. Bulan dan week terisi otomatis dari tanggal.";
    }
    renderList();
  }
  ...
  function renderList() {
    ...
    const items = records();
    countEl.textContent = items.length + " DOP";
    listEl.innerHTML = "";
    ...
    if (!visible.length) {
      ...
      empty.textContent = items.length ? "Tidak ada DOP yang cocok." : "Belum ada DOP.";
    ...
    visible.forEach(function (item) {
      ...
      editBtn.title = "Ubah DOP";
      editBtn.setAttribute("aria-label", "Ubah DOP");
      ...
      delBtn.title = "Hapus DOP";
      delBtn.setAttribute("aria-label", "Hapus DOP");
  ...
  function applyRecordToStore(record) {
    ...
    if (!store.source_file) store.source_file = "Form input DOP Karhutla";
    return items.indexOf(record);
  }

  async function persistStore() {
    if (!window.KarhutlaDopStore) {
      throw new Error("Modul simpan DOP belum termuat.");
    }
    return KarhutlaDopStore.save(dataStore());
  }
  ...
  async function deleteRecord(index, row) {
    ...
    try {
      if (index < 0 || index >= store.records.length) {
        setStatus("DOP tidak ditemukan.", "is-err");
        return;
      }
      ...
      setStatus("DOP dihapus. Total " + store.total_records + " DOP.", "is-ok");
    } catch (err) {
      store.records = snapshot;
      store.total_records = snapshotTotal;
      renderList();
      setStatus(err.message || "Gagal menghapus DOP.", "is-err");
    }
  }
    ...
    try {
      if (window.KarhutlaDopStore) {
        for (let i = 0; i < pendingPhotos.length; i += 1) {
          pendingPhotos[i].uploaded = await KarhutlaDopStore.uploadPhoto(
      ...
      setStatus(
        ...
        " Total " + dataStore().total_records + " DOP." +
        " Buka Hotspot untuk melihat titik DOP ini pada peta.",
        "is-ok"
      );
    } catch (err) {
      store.records = snapshot;
      store.total_records = snapshotTotal;
      renderList();
      setStatus(err.message || "Gagal menyimpan DOP.", "is-err");
    } finally {
  ...
  function downloadAllExcel() {
    try {
      ...
      const items = records();
      if (!items.length) {
        setStatus("Belum ada DOP untuk diunduh.", "is-err");
        return;
      }
      ...
      XLSX.utils.book_append_sheet(wb, ws, "DOP Karhutla");
      XLSX.writeFile(wb, "DOP Karhutla " + stamp() + ".xlsx");
      setStatus("Excel " + items.length + " DOP berhasil diunduh.", "is-ok");
    } catch (err) {
    ...
    try {
      ...
      if (activeIndex >= 0 && records()[activeIndex]) {
        ...
        if (!drafted.tanggal || !drafted.site || !drafted.lokasi) {
          setStatus("Pilih DOP di daftar, atau isi tanggal, site, dan lokasi terlebih dahulu.", "is-err");
          return;
        }
        row = drafted;
      }
      ...
      applyColWidths(ws, [18, 12, 12, 14, 14, 28, 28, 20, 24, 40, 14]);
      const used = {};
      const name = uniqueSheetName((row.site || "DOP") + " " + formatDateId(dayKey(row.tanggal)), used);
      XLSX.utils.book_append_sheet(wb, ws, name);
      XLSX.writeFile(wb, "DOP Karhutla - " + name + ".xlsx");
      setStatus("Excel DOP \"" + name + "\" berhasil diunduh.", "is-ok");
    } catch (err) {
      setStatus(err.message || "Gagal mengunduh Excel.", "is-err");
    }
  }
  ...
  function applyOpenId() {
    ...
    if (window.history && window.history.replaceState) {
      window.history.replaceState({}, "", "dop.html");
    }
    fillForm(list[index], index);
    setStatus("Membuka DOP yang dipilih dari peta Hotspot.", "is-ok");
    return true;
  }

  function boot() {
    if (!applyOpenId()) resetForm();
    renderList();
  }

  if (window.KarhutlaDopStore) {
    KarhutlaDopStore.hydrate().then(boot).catch(boot);
  } else {
    boot();
  }
})();
